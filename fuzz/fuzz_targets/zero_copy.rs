...
use bincode_next::relative_ptr::{
    ZeroCopyBuilder, ZeroBuilder, DeepValidator, ZeroSlice, 
    ZeroString, FixedString, ZeroStr, RelativeBuilder
};

#[derive(Arbitrary, ZeroCopy, bincode_next::Decode, bincode_next::Encode, PartialEq, Debug, Clone, Copy)]
#[repr(C)]
pub struct Inner {
    pub a: u32,
    pub b: bool,
...
fuzz_target!(|data: &[u8]| {
    ...
    let id = match <[u8; 16]>::arbitrary(&mut u) { Ok(v) => v, Err(_) => return };
    let inner_vec = match <Vec<Inner>>::arbitrary(&mut u) { Ok(v) => v, Err(_) => return };
    let name_str = match <String>::arbitrary(&mut u) { Ok(v) => v, Err(_) => return };
    let dyn_name_str = match <String>::arbitrary(&mut u) { Ok(v) => v, Err(_) => return };
    ...
    let data_slice = inner_vec.iter().cloned().map(|i| InnerBuilder {
        a: i.a,
        b: i.b,
        padding: i.padding,
    }).collect::<Vec<_>>().build_to_target(&mut builder, root_offset + 16);
    ...
    assert!(resolved_root.is_valid_deep(&buffer), "Derived deep validation failed on valid structure");
    ...
    let arbitrary_root = match <Root>::arbitrary(&mut u) { Ok(v) => v, Err(_) => return };
