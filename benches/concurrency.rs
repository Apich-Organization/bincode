...
pub fn bench_concurrency(c: &mut Criterion) {
    ...
    let encoded_next = Arc::new(bincode_next::encode_to_vec(&payload, config::standard()).unwrap());
    ...
    let encoded_for_async = Arc::new(rt.block_on(serialize_for_async_bincode(&payload)));
