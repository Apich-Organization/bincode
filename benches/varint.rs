...
fn slice_varint_u8(c: &mut Criterion) {
    ...
    let dist = Uniform::new(0, u8::MAX).expect("Invalid range");
...
fn slice_varint_u16(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    let dist = Uniform::new(0, u16::MAX).expect("Invalid range");
...
fn slice_varint_u32(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    let dist = Uniform::new(0, u32::MAX).expect("Invalid range");
...
fn slice_varint_u64(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    let dist = Uniform::new(0, u64::MAX).expect("Invalid range");
